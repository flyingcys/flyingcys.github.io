class SerialTerminal {
    ...
    checkWebSerialSupport() {
        // 检测浏览器类型
        const isChromeBased = window.isChromiumBrowser();
    ...
    processReceiveBuffer() {
        ...
        for (let i = 0; i < this.receiveBuffer.length; i++) {
            ...
            if (char === '\n' || char === '\r') {
                ...
                if (line.length > 0) {
                    // 限制单行最大长度，防止显示异常
                    if (line.length > window.MAX_DISPLAY_LINE_LENGTH) {
                        // 将过长的行分割成多行
                        while (line.length > window.MAX_DISPLAY_LINE_LENGTH) {
        ...
        if (this.receiveBuffer.length > window.MAX_BUFFER_LENGTH) {
            if (this.receiveBuffer.trim()) {
    ...
    async sendData() {
        ...
        try {
            ...
            if (this.hexModeCheck.checked) {
                // HEX模式
                dataToSend = window.hexStringToBytes(input);
            } else {
                // 文本模式
    ...
    addToDisplay(text, type = 'received') {
        ...
        // 对显示文本进行安全处理
        const safeText = window.sanitizeDisplayText(text);
        ...
        if (this.showTimestampCheck && this.showTimestampCheck.checked) {
            const timestamp = window.generateTimestamp();
    ...
    async startFlashDownload() {
        ...
        try {
            ...
            // 读取文件数据
            const fileData = await window.readFileAsArrayBuffer(this.selectedFile);
        ...
        this.quickCommands.forEach((command, index) => {
            ...
            item.innerHTML = `
                <div class="command-info">
                    <div class="command-name">${window.escapeHtml(command.name)}</div>
                    <div class="command-value">${window.escapeHtml(command.value)}</div>
                </div>
    ...
    editCommand(index) {
        ...
        item.innerHTML = `
            <div class="command-info">
                <input type="text" class="command-name" value="${window.escapeHtml(command.name)}" maxlength="20">
                <input type="text" class="command-value" value="${window.escapeHtml(command.value)}">
            </div>
            <div class="command-actions">
    ...
    handleSerialTargetDeviceChange() {
        ...
        // 定义设备对应的波特率配置
        const deviceBaudrateConfig = window.DEVICE_BAUDRATE_CONFIG;
